...
analytics.service('AnalyticsService', function($log, $window, $mixpanel, SettingsModel) {
  ...
  this.logEvent = (message, data) => {
    ...
    // Mixpanel expects a flat object
    const flattenedData = data ? flatten(data, {
      delimiter: ' ',
      safe: true
    }) : undefined;

    const startCaseFlattenedData = flattenedData ? _.mapKeys(flattenedData, (value, key) => {
      return _.startCase(key);
    }) : undefined;

    if (SettingsModel.get('errorReporting') && isRunningInAsar()) {
      $mixpanel.track(message, _.mapKeys(flattenedData, _.startCase));
    }

    if (startCaseFlattenedData) {
      message += ` (${JSON.stringify(startCaseFlattenedData)})`;
    }
